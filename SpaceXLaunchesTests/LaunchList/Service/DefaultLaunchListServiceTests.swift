...
final class DefaultLaunchListServiceTests: XCTestCase {
    ...
    func test_fetchRockets_WhenFetcherFail_ShouldFail() async {
        let expectedError = ServiceError.badURL
        fetcherMock.fetchFromEndpointImpl = { _ in .failure(expectedError) }
        let result = await sut.fetchRockets()
        XCTAssertEqual(result, .failure(expectedError))
    }
    
    func test_fetchRockets_WhenFetcherSucceeds_WhenDecodeFails_ShouldFailWithDecode() async throws {
        let wrongData = try XCTUnwrap("wrong data".data(using: .utf16))
        fetcherMock.fetchFromEndpointImpl = { _ in .success(wrongData) }
        let result = await sut.fetchRockets()
        XCTAssertEqual(result, .failure(.decode))
    ...
    func test_fetchRockets_WhenFetcherSucceeds_ShouldSucceed() async throws {
        let expectedRocket = [SpaceXRocket.stub()]
        let encodedRocket = try XCTUnwrap(JSONEncoder().encode(expectedRocket))
        fetcherMock.fetchFromEndpointImpl = { _ in .success(encodedRocket) }
        let result = await sut.fetchRockets()
        XCTAssertEqual(result, .success(expectedRocket))
    }
}
